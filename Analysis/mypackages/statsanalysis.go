...
func Examine(num_of_stats int64) {
...
func DisplayData() {
	...
	if percent_deviated > 0 {
		fmt.Print("- Filecreate latency increased by ", math.Round(math.Abs(percent_deviated)), "% [current value at ",Response_for_elastic.FileLatency ," ms].")
	}else{
		fmt.Print("- Filecreate latency decreased by ", math.Round(math.Abs(percent_deviated)), "% [current value at ",Response_for_elastic.FileLatency ," ms].")
	}
	...
	if percent_deviated > 0 {
		fmt.Print(" Also, System Utilization(Space) change rate increased by ", math.Round(math.Abs(percent_deviated)), "% [current value at ",Response_for_elastic.SystemUtilizationChangeRate," GiB/sec]" )
	} else {
		fmt.Print(" Also, System Utilization(Space) change rate decreased by ", math.Round(math.Abs(percent_deviated)), "% [current value at ",Response_for_elastic.SystemUtilizationChangeRate," GiB/sec]" )
	}
	...
	if percent_deviated > 0 {
		fmt.Println(" and Cluster Space Utilization increased by ", math.Round(math.Abs(percent_deviated)), "% [current value at ", Response_for_elastic.ClusterSpaceUtilization ," TiB]"  )
	} else {
		fmt.Println(" and Cluster Space Utilization decreased by ", math.Round(math.Abs(percent_deviated)), "% [current value at ", Response_for_elastic.ClusterSpaceUtilization ," TiB]")
	}
	fmt.Println()

	fmt.Println("- Meta Data Utilisation stands at ", Response_for_elastic.MetaDataSpacePercentage ,"% with a current value of",Response_for_elastic.MetaDataUtilization ,"TiB" )
	fmt.Println()
	// 	if !MetricDataAnalysis(float64(Response_for_elastic.FileCreateSum), Stats_response.FileCreateSumStats.SD, Stats_response.FileCreateSumStats.PastData) {
	// 		percent_deviated = PercentageCalculator(GetMedian(Stats_response.FileCreateSumStats.PastData), float64(Response_for_elastic.FileCreateSum))
	// 		if percent_deviated > 0{
...
func UserRequestForData(starttime, endtime int64) {
	...
	fmt.Println("Average BackUp Time : ", ConvertUnixTime(int64(jobbackup_mean)))
	fmt.Println("Average Archival Time : ", ConvertUnixTime(int64(jobarchival_mean)))
	fmt.Println("Average Replication Time : ", ConvertUnixTime(int64(jobreplication_mean)))
	fmt.Println("Average File Create Rate : ", filecreaterate_mean)
	fmt.Println("Average File latency : ", filelatency_mean)
	fmt.Println("Average System Utilization Change : ", SystemUtilizationChangeRate_mean)
	fmt.Println("Average Cluster Space Utilization : ", ClusterSpaceUtilization_mean)
	fmt.Println("Average Garbage Collection : ", GarbageCollection_mean)
	fmt.Println("Average Meta Data Utilization : ", MetaDataUtilization_mean)
	fmt.Println("Average Deduplication : ", Deduplication_mean)
	fmt.Println()
	fmt.Println("The jobs which have been completed duirng this period are:")
	fmt.Println()
	for k, _ := range ProtectionMap {
		fmt.Println("	Job Name : ", k)
		fmt.Println("	Average BackUp Time : ", ConvertUnixTime(int64(ProtectionMap[k].JobBackUpTimeStats.Mean)))
		fmt.Println("	Average Archival Time : ", ConvertUnixTime(int64(ProtectionMap[k].JobArchivalTimeStats.Mean)))
		fmt.Println("	Average Replication Time : ", ConvertUnixTime(int64(ProtectionMap[k].JobReplicationTimeStats.Mean)))
		fmt.Println("	Total runs : ", len(ProtectionMap[k].JobBackUpTimeStats.PastData))
		fmt.Println()
